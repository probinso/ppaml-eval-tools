...
\item as \t24, I want to submit a \sol, so that it can be evaluated
  ...
  \item \t24 packages the \sol
    \begin{enumerate}
    \item \G needs conformant run.sh
      \begin{enumerate}
      \item ./run.sh $<$config-file$>$  $<$input-dir$>$  $<$output-dir$>$  $<$log-file$>$ 
      \item FOR $<$config-file$>$ WE WILL NO LONGER ACCEPT /dev/null, ONLY EXISTING FILES
      \item run.sh must use \eng environment variables
      \item \G guarantees environment variables
        \begin{enumerate}
        \item \eng root directory  of \eng at location \$\{ENGROOT\}
        \item temporary work space \$\{ENGTMP\}
        \end{enumerate}
      \end{enumerate}
    \item \G needs a \sol-smoketest configuration file
      \begin{enumerate}
      \item minimal iterations
      \item estimated time and space requirements
      \end{enumerate}
    \item \G needs a configuration file to be used in evaluations
      \begin{enumerate}
      \item estimated time and space requirements
      \end{enumerate}
    \end{enumerate}
...
\item as a \de, I have been selected to submit a \cp, so that it can be used in the evaluation process
  ...
  \item \G reviews and approves the \cp and \ds{s}
    ...
    \item \G runs updatadataset.sh to registers the new \ds{s} to \ptk from Midas
    \item \G notifies \t24
    \end{enumerate}
...
\item as \G or \t24 I want to run a \sol against a \ds for a \cp in \ptk, so that I can later run a \tev
  ...
  \item \G takes inventory of prerequisites:
    ...
    \end{enumerate}
  \item using the \ptk run the \sol, so that later, we can run all appropriate \sol:\ds pairs
    ...
    \item save generated \desc for debugging purposes
    \end{branch}
