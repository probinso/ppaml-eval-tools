# slamutil.py -- 2D EKM SLAM utilities
...
def FindClosestLandmark(robotState,lasrange,bearing,landmarks,phase,FindGlobalLaserCoord_phase):
  with phase.running():
    if len(landmarks) == 0:
      return False
    measurementPoint = FindGlobalLaserCoord(robotState,lasrange,bearing,FindGlobalLaserCoord_phase)
...
def FindGlobalLaserCoord(v_state,r,bearing,phase):
  with phase.running():
    phi = v_state[2]
...
################################################################################
def ClumpsToRangeBearing(clumps, phase):
  with phase.running():
    ranges = []
    bearings = []
    for i in clumps:
...
def FindClumps(laser,intensity,phase):
  with phase.running():
